...
├── results/                 # Stage 7 robustness tables (CSV + LaTeX, generated)
│
...
An additional sensitivity report excludes individual label-3 votes (and blanks), keeping each expert's other votes. It uses generalized Fleiss' kappa for variable rater counts, following the unweighted [irrCAC formulation](https://github.com/kgwet/irrCAC/blob/master/R/agree.coeff3.raw.r). Observed agreement averages pair-specific agreement over pairs with at least two directional votes. Chance agreement uses the squared mean within-pair category proportions over pairs with at least one directional vote. Single-vote pairs contribute only to chance agreement; zero-vote pairs contribute to neither. Output includes counts for zero, one, two and three retained votes and is saved for Overall/Coarse/Fine to `results/expert_fleiss_kappa_excluding_3.csv`. This sensitivity analysis does not alter consensus labels or accuracy scoring.
...
Each attenuation expert CSV contains 77 pairs. Pairs `(13330, 678)`, `(14144, 18846)`, and `(18844, 7083)` were removed from all three CSVs because attenuation deltas were unavailable. The Excel workbooks retain their 80 pairs. The four originally blank labels in expert 1's CSV and the one in expert 3's CSV are retained as missing votes; their temporary recoding to 3 was undone. Rerun validation to refresh outputs after label edits.
...
### results/ — stage 7 only, generated by `--robustness`

| File | Description |
|------|-------------|
| `permutation_test.csv` | Per-permutation expert accuracy and modulator correlations |
| `permutation_summary.csv` | Observed value, permuted mean/SD, and permutation p-value per metric |
| `delta_agreement.csv` | Pairwise agreement between the deltas produced at different `s` |
| `modulator_corrs.csv` | Δ-versus-`D_misc` correlations recomputed at each `s` |
| `expert_accuracy.csv` | Overall expert paired-comparison accuracy at each `s` |
| `robustness_tables.tex` | All of the above as paste-ready LaTeX tables |
