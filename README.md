...
│   ├── robustness.py        # Stage 7: Permutation control and s-sensitivity (opt-in)
│   ├── splits.py            # Professor-level train/test split (shared by 4, 5, 7)
...
│
├── Paper/                   # Manuscript sources, figures, and paper-maintenance scripts
│   ├── todo.md              # Outstanding paper work
│   └── worked_examples.py   # Regenerates the Section 3.9 worked examples at the current s
│
└── sidequestz/              # Standalone analyses awaiting a home in the pipeline
```

`handoff.md` in the project root records the current state of the work, what has
changed since the last full pipeline run, and what still needs regenerating.
...
Stage 7 (robustness) is **not** part of a default run — see `--robustness` below.
...
| `--robustness` | Run the stage 7 robustness analyses (permutation control and s-sensitivity tables). Off by default: the permutation control re-attenuates the corpus once per permutation. |
...
# Full pipeline plus the robustness analyses
python pipeline.py --robustness --visualize

# Generate plots from existing outputs without re-running the pipeline
python pipeline.py --visualize-only
```

### Two-pass `s` selection

`--optimize-s` prints the tuned `s` and uses it for that run, but does not write it
back to `constants.py`. If the tuned value differs from `S_VALUE` the stage warns
rather than failing. To adopt a new value: run with `--optimize-s`, edit
`S_VALUE` in `constants.py`, then rerun the pipeline so every stage agrees.
...
python -m src.robustness --skip-permutation             # sensitivity tables only
python -m src.robustness --n-permutations 20            # shorter permutation run
python -m src.robustness --s-values 0.4 0.83            # custom sensitivity points
```

### Paper maintenance

```bash
python Paper/worked_examples.py          # regenerate the Section 3.9 worked examples
python Paper/worked_examples.py --s 0.83 # at a specific exponent
```

Run this whenever `s` changes. The Section 3.9 examples are hand-traced prose and
are the easiest thing in the paper to leave stale. The script also acts as a
cross-file consistency check, warning if the pipeline's outputs were produced at a
different `s` than the one currently configured.
...
**`src/splits.py`** — single source of truth for the 80/20 professor-level split. Defined once and reused by regression (model training), attenuation (`s` selection) and the reporting visualizations, so the three stages cannot drift apart. Splitting is on professor rather than review because reviews of the same professor are correlated; a review-level split would leak across the boundary.
...
No `eval_set` is passed to either fit, and there is no early stopping: every model runs the full 1000 iterations and is scored only on rows that took no part in fitting. An `eval_set` both sizes the model and is then scored, which biases whatever is reported on it — and passing the test professors would additionally mean the model producing the attenuation deltas had been sized using the very rows Section 4.3 reports.

**`src/robustness.py`** — stage 7, opt-in. Three independent analyses. The **permutation control** shuffles `D_misc` across reviews, breaking the correspondence between a review's measured off-topic density and the attenuation it receives while leaving every marginal distribution intact, then recomputes expert accuracy and the modulator correlations against the *true* densities. It is the control condition establishing that the mechanism's behaviour depends on real off-topic content rather than on any arbitrary review-specific adjustment. The **s-sensitivity tables** recompute delta agreement, modulator correlations and expert accuracy across several values of `s`, showing the framework's behaviour is not an artifact of the exponent the grid search happened to select. Expert accuracy across `s` uses the same evaluable pairs and Coarse/Fine membership defined at `s=1.0`, with bin counts shown alongside accuracy and no binomial p-values. Each setting changes predictions while keeping these comparison groups fixed. The **professor bootstrap** resamples held-out professors and reports the mean, SD, and 95% percentile interval for each modulator correlation. Loads the trained model without refitting and prints all summary reports to the console.
...
**`src/validation.py`** — evaluates attenuation against three experts. Run `python -m src.validation` using the three expert CSVs in `data/raw/`. Pairs are aligned by review IDs, including reversed pair orientations; output labels follow expert 1's orientation where available. At least two matching votes establish a majority. A model prediction is correct when it matches a majority for label 1 or 2. Pairs without a majority and pairs with a label-3 majority count as incorrect and enter the accuracy denominator when model deltas are available. Blank votes are missing; label 3 is non-directional (both valid). Only pairs with unavailable model deltas are excluded from majority-vote accuracy. Exact model ties retain the existing review-1 selection rule. These penalties also apply to confidence intervals and robustness accuracy; they do not alter the expert agreement calculations. Individual-expert accuracy also counts label-3 votes as incorrect.

Fleiss' kappa uses labels 1, 2 and 3 on pairs with all three ratings, including pairs without model deltas. Accuracy is reported for consensus and each individual expert, overall and for the disjoint fine `[0, COARSE_DELTA_THRESHOLD)` and coarse `[COARSE_DELTA_THRESHOLD, infinity)` bins. Individual-expert accuracy counts non-directional (label-3) votes as incorrect and excludes only that expert's blank votes and pairs with unavailable model deltas. Each accuracy includes a two-sided 95% Wilson interval. Intervals assume independent pairs and condition on these three experts; they do not quantify uncertainty from sampling a wider population of experts. Methods: [Fleiss' kappa](https://www.statsmodels.org/v0.13.5/generated/statsmodels.stats.inter_rater.fleiss_kappa.html), [SciPy Wilson intervals](https://docs.scipy.org/doc/scipy-1.16.1/reference/generated/scipy.stats._result_classes.BinomTestResult.proportion_ci.html).

Validation prints consensus and individual-expert accuracy, Wilson intervals, both kappa summaries, and counts of missing deltas, missing ratings, and model ties to the console. It does not export pair-level records or report files. The optional `sidequestz/expert_confidence_intervals.py` exporter uses the same calculations and separately writes CSV and LaTeX tables to `sidequestz/output/`. Robustness runs also use the shared majority-vote denominator.

Fleiss' kappa is reported overall and separately for coarse and fine pairs, recomputing chance agreement within each subset. Overall includes pairs without model deltas; those pairs cannot be assigned to coarse/fine bins. Each row reports total pairs, complete three-expert pairs, and pairs excluded for missing ratings. Kappa does not filter on majority agreement or model correctness.

An additional sensitivity report excludes individual label-3 votes (and blanks), keeping each expert's other votes. It uses generalized Fleiss' kappa for variable rater counts, following the unweighted [irrCAC formulation](https://github.com/kgwet/irrCAC/blob/master/R/agree.coeff3.raw.r). Observed agreement averages pair-specific agreement over pairs with at least two directional votes. Chance agreement uses the squared mean within-pair category proportions over pairs with at least one directional vote. Single-vote pairs contribute only to chance agreement; zero-vote pairs contribute to neither. Output includes counts for zero, one, two and three retained votes and is printed for Overall/Coarse/Fine. This sensitivity analysis does not alter consensus labels or accuracy scoring.

The three expert CSV paths are held in the single `EXPERT_LABELS_PATH` constant. The expert 2/3 Excel workbooks and CSVs use expert 1's current review IDs. IDs were reconciled using unique review text, normalizing the one doubled `[[profanity]]` placeholder for matching. Review text, labels and notes were preserved.

Each attenuation expert CSV contains 77 pairs. Pairs `(13330, 678)`, `(14144, 18846)`, and `(18844, 7083)` were removed from all three CSVs because attenuation deltas were unavailable. The Excel workbooks retain their 80 pairs. The current CSVs contain no blank votes. Expert 1's label for pair (7645, 4704) is corrected to 3 (both valid). Rerun validation to refresh the console report after label edits.
...
### Console reports: validation and robustness

Validation prints accuracy with Wilson intervals, expert agreement, and exclusion counts. Stage 7 (`--robustness`) prints sensitivity tables, permutation-control summaries, and professor-bootstrap summaries. These stages do not write CSV or LaTeX reports. Per-pair validation records and individual permutation/bootstrap draws are not printed or saved.
