...
\auth{A post hoc diagnostic within the fine condition found agreement on 16/19 unanimous pairs (84.2\%) and 5/15 split-vote pairs (33.3\%): ten of thirteen model--majority disagreements coincided with expert disagreement. Three remained despite unanimity, at absolute-adjustment contrasts of 0.314, 0.005, and 0.026 points.}

\subsubsection{Permutation Control}

\auth{We shuffled non-zero $D_{\text{misc}}$ values among reviews 1{,}000 times, preserving zeros and the 77-pair analytic population, and recomputed deltas with the fitted model held fixed. Density was shuffled both as a model feature and as the attenuation input; modulator correlations used the original densities. The original test recomputed coarse/fine membership after each shuffle. A supplementary sensitivity analysis held membership fixed at the observed $s=1$ groups, using the same shuffles and predictions. Table~\ref{tab:permutation-control} reports both specifications.}

\begin{table}[htbp]
\caption{\auth{Permutation control ($N=1{,}000$). Expert rows report agreement; modulator rows report Pearson $r$. Null entries give the mean and one-sided $p$ using $(k+1)/(N+1)$. Recomputed coarse groups contained a mean of 10.51 pairs (range 3--20), compared with 43 observed; fine groups averaged 66.49, compared with 34 observed. Fixed-bin subgroup analyses are supplementary; overall and modulator tests are shared.}}
...
\auth{Overall expert agreement exceeded the shuffled-density benchmark ($p=.001$). Recomputed coarse groups were smaller and changed composition, retaining high null agreement; 179 shuffles reached 100\% ($p=.180$). On the original 43 coarse pairs, no shuffle matched observed agreement ($p=.001$). Neither fine specification rejected its respective null ($p=.378$ recomputed; $p=.321$ fixed), with observed advantages over null means of 1.9 and 5.5 percentage points.}
...
\auth{Table~\ref{tab:sensitivity}(b) holds expert-pair membership and coarse/fine bins fixed at $s=1$, identical to the primary expert analysis. Overall agreement ranged from 83.1\% to 88.3\% across the tested exponents ($n=77$). Coarse agreement remained 100\%, while fine agreement varied from 61.8\% to 73.5\% (21--25 of 34 pairs). Aggregate adjustment patterns were relatively stable, with greater sensitivity in close comparisons.}

\auth{Table~\ref{tab:sensitivity}(d) reports 95\% confidence intervals from 1{,}000 bootstrap resamples of held-out professors. All four correlations have narrow intervals (SD $\leq 0.027$), indicating that the modulator relationships are robust.}
...
\auth{For RQ3, bootstrap resampling and exponent sensitivity support stability of aggregate coherence relationships, while permutation controls show dependence on the original density assignment ($p=.001$). Expert comparisons provide strong alignment on large adjustment contrasts: the original coarse group exceeded its shuffled benchmark in the supplementary fixed-bin analysis, whereas recomputed coarse groups were smaller and retained high null agreement. Neither fine specification rejected its respective null. Fine disagreements concentrated in split-vote pairs, although three occurred despite unanimity. Disagreement may reflect differences between experts' holistic validity judgments and the taxonomy-based relevance operationalized by the method, or limitations of the mechanism at small adjustment contrasts; the present design cannot distinguish these explanations.}

\auth{The fine-condition agreement and permutation benchmarks provide a reproducible baseline for evaluating subsequent improvements; small adjustment differences should not be interpreted as established validity orderings. These findings do not establish that the taxonomy or density measure is correct, nor that adjusted ratings are validated measures of teaching quality.}
