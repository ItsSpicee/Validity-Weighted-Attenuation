...
\item[RQ3.] How does the mechanism's adjustment ordering align with experts' relative-validity judgments on selected review pairs?
\end{description}
\end{authcontent}
...
density-only ordering reproduces on the same pairs. We regard these results as establishing the mechanism's coherence and
and a reproducible evaluation resource.}
%========================================================================================
...
\auth{Table~\ref{tab:ex2} presents an end-to-end trace of an individual review with high \textit{Miscellaneous} density ($D_{\text{misc}}$=0.670), where affect in the \textit{Miscellaneous} category dominates, receiving a positive adjustment of $+$0.427.} 
...
To assess whether the four-category taxonomy is conceptually distinct, two independent LLMs (GPT-5 mini and Grok 4.1) categorized 386 clauses, yielding Cohen's \citeyear{Cohen1960} $\kappa = 0.7273$ (``substantial agreement''; \citeNP{Landis1977}), supporting the clarity of category boundaries.
...
Over all 17,127 reviews, 10,375 (60.58\%) were adjusted, with a mean $|\Delta|$ of 0.1964 and a mean signed $\Delta$ of 0.0440 (Wilcoxon signed-rank $W = 24{,}260{,}091$, $p < .001$); the professor-level distribution shape remained largely consistent. Adjustments form a two-sided tail: individual values range from $-1.39$ to $+2.71$ rating points, with the largest magnitudes concentrated in reviews combining high taxonomy-excluded density with intense off-taxonomy affect. The professor-level distribution shape remained largely
consistent.
...
\auth{Table~\ref{tab:sensitivity}(c) shows that absolute Spearman correlations increase monotonically with $s$ across
the tested range, while the Pearson correlations remain nearly constant. This is
consistent with the functional form: as $s$ increases, $\zeta = 1 -
D_{\text{misc}}^{s}$ approaches 1 everywhere except near $D_{\text{misc}} = 1$,
so the adjustment ordering increasingly reflects the reviews at the measured
density maximum. The positive and negative relationships remain in the same directions throughout.}
...
Addressing RQ2, held-out coherence checks show that attenuation reduces model-estimated feature attribution associated with taxonomy-excluded affect ($-9.6\%$) while shifting attribution toward pedagogical categories ($+1.6\%$ to $+2.5\%$); correlations with pedagogical categories are maintained or slightly strengthened. Adjustment magnitude tracks \textit{Miscellaneous} density in both directions, with weaker but directionally consistent emotion-intensity relationships. The average absolute adjustment of $0.1964$ points and preserved professor-level distribution shape are consistent with the conservative character advocated by \citeA{MarshRoche1997}, though the adjustment distribution has a two-sided tail (maximum $+2.71$, minimum $-1.39$). The extremes arise by construction: reviews with $D_{\text{misc}} = 1$ have all affect features zeroed under attenuation, so their attenuated predictions revert to a fixed model-predicted baseline ($\hat{y} = 3.65$ on the 1--5 scale), and their adjustments equal the removal of the affect's original modelled contribution in
full.
...
assignment: no shuffle produced correlations as extreme as observed ($p=.001$ each), with the observed values ($r = .687$, $-.708$) well above the permutation floor ($r = .233$, $-.158$), conditional on the fitted model and the specified
...
attenuation input. Professor-level bootstrap CIs ([.651, .720] and [$-.741$,
 $-.671$]) confirm stability across held-out professor samples. Across attenuation
exponents $s \in \{0.5, \ldots, 1.5\}$, Pearson modulator correlations are nearly
unchanged and adjacent exponents yield near-identical adjustments ($r > .995$,
mean $|\Delta_a - \Delta_b| < 0.03$); $s=1$ was selected a priori on proportionality grounds. The
Mann--Whitney rank-biserial ($|r| = .789$), computed on deliberately separated
density groups ($D_{\text{misc}} > 0.5$ vs.\ $D_{\text{misc}} < 0.2$), indicates
that a high-density review receives a larger-magnitude adjustment than a
low-density review in approximately 89\% of cross-group pairings. Together, these
checks found no evidence that the observed coherence depends on a particular
professor sample, exponent choice, or density assignment.}

\auth{Addressing RQ3, adjustment-magnitude ordering agreed with expert
relative-validity judgments on 83.1\% of 77 sampled pairs (100\% coarse, 61.8\%
...
under either bin specification ($p = .378$ recomputed, $p = .321$ fixed); close-pair alignment is not established at this sample size. Expert agreement varied across exponents within the respective Wilson intervals
(fine 61.8--73.5\%).}
...
between the experts' broader validity judgments and this study's taxonomy. The
fine-condition results and the density-only match provide a reproducible baseline:
subsequent improvements to the taxonomy, classification, or density measure should
be evaluated for what they add beyond density, while the usefulness of signed
rating adjustments and audit trails as additional outputs requires separate
evaluation.}
...
\auth{Because attenuation removes the modelled affective contribution in full at
 $D_{\text{misc}} = 1$, wholly off-taxonomy negative reviews receive upward
adjustments and wholly off-taxonomy positive reviews downward adjustments, toward
the model's zero-affect baseline. This is the intended limiting behaviour of the
mechanism, is visible in each review's audit trail, and should be inspected,
not automated, wherever a single heavily adjusted review would carry weight.}
...
pairs. This comparison does not establish the ordering advantage of the
model-based mechanism, which remains unconfirmed at this pair-set design; the
mechanism's other outputs -- calibrated adjustment magnitudes and per-review
traceability -- are not provided by density alone and require separate
evaluation.}
