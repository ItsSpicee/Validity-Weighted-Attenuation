...
\item[RQ3.] How do adjustment-magnitude and density-only orderings compare in their agreement with experts' relative judgments of review validity and topical relevance on selected review pairs?
\end{description}
\end{authcontent}
...
\auth{Density describes the proportion of review text assigned outside the stated taxonomy, but does not indicate how the fitted rating model responds to the affect expressed in that content. Validity-weighted attenuation connects this content-based measure to a specified transformation of the model's inputs: $D_{\text{misc}}$ determines the degree of scaling applied to the \textit{Miscellaneous} emotion features, and the prediction difference measures the model's response. Density and attenuation therefore provide complementary information about the prevalence of taxonomy-excluded content and the prediction's sensitivity to scaling its affective representation. The down-weighting factor $\zeta$ is calculated as:}
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.375\linewidth}>{\raggedright\arraybackslash}p{0.375\linewidth}}
\toprule
 & \shortstack[l]{\textbf{Pair 1}\\\textit{Attenuation-only success}} & \shortstack[l]{\textbf{Pair 6}\\\textit{Density-only success}} \\
\midrule
\multicolumn{3}{l}{\textit{Decision signals}} \\
...
Attenuation selects & \textbf{Rev.\ 2} \checkmark & Rev.\ 1 $\times$ \\
Density selects & Rev.\ 1 $\times$ & \textbf{Rev.\ 2} \checkmark \\
\midrule
\multicolumn{3}{l}{\textit{Full anonymized review text}} \\
\textbf{Review 1} &
{\footnotesize ``[Instructor] is a nice guy and he knows math rather well. However, due to his accent it is hard to understand him very well and it is hard to learn from it. He does not respond to emails very quickly, and he loses some assignments if you turn them in outside of class. He tries to have a sense of humor but it's more dad jokes, really.''} &
{\footnotesize ``Dr.\ [Instructor] is honestly one of the kindest people I've ever met. His lectures are very interesting and easy to understand. Tests are really fair.''} \\
\addlinespace[2pt]
\textbf{Review 2} &
{\footnotesize ``The reading assignments were ridiculously long and a lot of them were very dry. But after the first few classes, you realized that he gives a synopsis of the assignment every day, so you don't really have to read them all. He's very popular during office hours, so get there early and be prepared to wait.''} &
{\footnotesize ``This man is a lunatic. He managed to turn a history class into a literature course. I left his class with less knowledge about the world post-1500s than when I went into it. He does care about his students. However, he has one teaching style and that is it. So if you are not on the same wavelength as him, prepare to be left in the dark.''} \\
\bottomrule
\end{tabular}
\end{table}

\auth{In Pair 1, densities of 0.21 and 0.27 accompany adjustment magnitudes of 0.222 and 0.005 rating points, respectively. Density selects Review 1, whereas attenuation selects Review 2, matching the unanimous expert judgment. This example shows that similar proportions of taxonomy-excluded text can accompany substantially different responses to the specified feature transformation. Pair 6 illustrates the converse disagreement: Review 1 has higher density but a smaller adjustment than Review 2, so attenuation selects Review 1 while density and all three experts select Review 2. A small adjustment therefore need not identify the review experts consider more valid or on topic. Together, these examples distinguish content prevalence from fitted-model sensitivity.}
...
\auth{Attenuation connects an explicit relevance taxonomy to a specified transformation of the fitted rating model's inputs. Whereas density measures the prevalence of taxonomy-excluded text, the signed prediction difference measures the direction and magnitude of the model's response to scaling the corresponding emotion features while holding other inputs fixed. Expressed in rating units, this response makes the consequences of the attenuation rule inspectable for individual reviews. Its interpretation remains conditional on the taxonomy, extracted features, fitted model, and specified transformation.}

\auth{Examining density and attenuation together distinguishes reviews with extensive taxonomy-excluded content but little prediction response from those whose predictions respond substantially to the specified transformation. Conversely, a smaller excluded proportion can accompany a larger response, as illustrated by the discordant pairs in Table~\ref{tab:discordant}. These patterns provide different grounds for inspecting the assigned topics, detected emotions, and resulting prediction change. Presenting both quantities alongside the original text and audit trail could therefore support examination of how content classification and model sensitivity interact. Whether this joint presentation improves stakeholders' interpretation of teaching feedback requires separate evaluation.}
...
Attenuation outcomes depend on upstream classifications and the fitted model: high $D_{\text{misc}}$ does not guarantee a large adjustment, and wholly \textit{Miscellaneous} reviews ($D_{\text{misc}}=1$) receive identical attenuated predictions ($\hat{y} = 3.65$) regardless of emotional content. \auth{In this implementation, $D_{\text{misc}}$ operationalizes taxonomy exclusion as the proportion of review words assigned to \textit{Miscellaneous}. Alternative operationalizations could use graded semantic alignment or uncertainty in topic assignment to determine attenuation weights.} Because the adjusted rating adds $\Delta$ to the raw rating without clipping, 1,643 reviews (9.6\%) fall outside the original 1--5 scale, ranging from $-0.31$ to $7.62$. These values function as unbounded model-based summaries for inspection rather than replacement scale scores.
