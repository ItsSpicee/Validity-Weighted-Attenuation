...
To answer our research questions, we develop and apply a five-stage NLP framework to 17,127 RMP student evaluations using clause-level aspect-term categorization, fine-grained emotion extraction, pedagogical density estimation, tree-based regression modelling, and validity-weighted attenuation. Rather than excluding emotional expression, the framework proportionally down-weights content \auth{classified outside of the stated pedagogical taxonomy}, preserving student evaluative intent while \auth{supplementing (was improving)} the signal-to-noise ratio of numerical ratings. Adjustments are auditable and grounded in textual evidence provided by the student, minimizing reliance on black-box systems. 
...
\begin{table}[H]
\caption{\auth{Aspect-term categorization taxonomy. Each pedagogical category is defined by natural-language descriptors encoded as sentence embeddings; representative examples are shown. Clauses that do not exceed the similarity threshold $\tau = 0.25$ for any pedagogical category are assigned to \textit{Miscellaneous}.}}
