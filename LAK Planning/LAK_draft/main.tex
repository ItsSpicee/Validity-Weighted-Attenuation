...
Existing computational SET research provides rich descriptive accounts of
...
\textbf{validity-informed attenuation}, an interpretable ABSA method that scales the
...
\item[RQ3.] How do adjustment-magnitude and density-only orderings compare in their agreement with experts' relative judgments of review validity and topical relevance on sampled review pairs?
\end{description}
...
\begin{itemize}
\item To our knowledge, the first auditable attenuation mechanism linking an explicit relevance taxonomy to traceable review-level adjustment records in a fitted rating model, motivated by validity and robust-estimation theory \cite{Messick1995,Huber1964}.
...
The remainder of this paper describes the mechanism and its audit trail
...
Density describes the proportion of review text assigned outside the stated taxonomy, but does not indicate how the fitted rating model responds to the affect expressed in that content. Validity-informed attenuation connects this content-based measure to a specified transformation of the model's inputs: $D_{\text{misc}}$ determines the degree of scaling applied to the \textit{Miscellaneous} emotion features, and the prediction difference measures the model's response. Density and attenuation therefore provide complementary information about the prevalence of taxonomy-excluded content and the prediction's sensitivity to scaling its affective representation. The down-weighting factor $\zeta$ is calculated as:
...
RoBERTa-GoEmotions outputs show higher positive-emotion label probabilities in high-rated reviews and higher negative-emotion label probabilities in low-rated reviews, with cross-valence signals at both extremes. One-star reviews also receive positive-emotion label probabilities (Figure~\ref{fig:one-star-emotion-heatmap}). These outputs may reflect mixed emotional expression, sarcasm, or classification error.
...
The dataset is split 80--20 by professor to prevent leakage and better reflect the difficulty of generalizing to unseen instructors. Table~\ref{tab:model-performance} reports 5-fold GroupKFold CV and held-out performance; the held-out MAE of 0.6347 falls inside the fold range, suggesting consistent generalization.
...
Table~\ref{tab:sensitivity}(d) reports 95\% confidence intervals from 1{,}000 bootstrap resamples of held-out professors. Across resamples, the four correlations had SDs of 0.027 or less, indicating stability across professor samples conditional on the fitted pipeline.
...
These patterns are stable under the tested perturbations. The permutation
...
The framework addresses similar CIV concerns to those documented by \citeA{Zhai2021} in controlled assessments, but in the considerably more challenging setting of informal, unmoderated SET reviews. This positions the work as initial method evidence for a validity-oriented direction in teaching-feedback analytics, rather than a ready intervention.
...
This study has no independent external criterion against which to compare whether adjusted ratings support more
...
\section{Conclusion}
%========================================================================================
Open-ended teaching feedback is an increasingly common learning-analytics
trace, yet computational approaches to it remain largely descriptive. This
research presented validity-informed attenuation, an auditable mechanism,
...
The framework preserves every original review and its full emotional
